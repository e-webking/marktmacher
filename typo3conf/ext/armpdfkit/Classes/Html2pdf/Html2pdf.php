<?php
namespace ARM\Armpdfkit\Html2pdf;

if (!defined('RELATIVE_PATH')) define('RELATIVE_PATH',''); 
if (!defined('FPDF_FONTPATH')) define('FPDF_FONTPATH',\TYPO3\CMS\Core\Utility\ExtensionManagementUtility::extPath('armpdfkit').'Classes/Html2pdf/font/');

include_once "Htmltoolkit.php";

class Html2pdf extends \ARM\Armpdfkit\Html2pdf\Fpdf {
	//internal attributes
	var $HREF; //! string
	var $pgwidth; //! float
	var $fontlist; //! array
	...
	public function __construct($orientation='P',$unit='mm',$format='A4') {
		...
		$this->SetFont('Arial','',10); // Changeable?(not yet...)
	...
	function CloseTag($tag)
	{
		...
		if($tag=='TITLE')	{
			$this->titulo=false; $this->blockjustfinished = true;
		}
				...
				case 'FONT-FAMILY': // one of the $this->fontlist fonts
					...
					for($i=0;$i<$fontarraysize;$i++)
					{
						...
						if ( in_array(strtolower($fonttype), $this->fontlist) ) {
							$this->SetFont(strtolower($fonttype));break;
						}
		...
		for( $i = 0 ; $i < $numrows ; $i++ )//rows
		{
			...
			for( $j = 0 ; $j < $numcols ; $j++ ) //columns
			{
				if (isset($cells[$i][$j]) && $cells[$i][$j])
				{
					...
					if($linesneeded > 8 && TYPO3_MODE == 'BE' && $nostyles_size < 450) {
						$linesneeded = 9;
					}
